...
test "collect if of" {
    ...
    const collect = try lex.collect_if_of(&[_]Token{ .K_Let, .K_Const });
...
test "collect if with peek" {
    const buf = "const";
    var lex = Lexer.new(buf, .{});

    const peeked = try lex.peek();
    _ = peeked;
    const collect = try lex.collect_if(Token.K_Const);
